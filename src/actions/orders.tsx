...
export async function emailOrderHistory(prevState: unknown, formData: FormData): Promise<{ message?: string; error?: string} > {
    ...
    const user = await db.user.findUnique({where: { email: result.data }, select: { email: true, orders: {
        select: { 
    ...
    const data = await resend.emails.send({
        ...
        react: <OrderHistoryEmail orders={await Promise.all(orders)}></OrderHistoryEmail>
    })
